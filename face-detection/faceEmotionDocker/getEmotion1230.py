...
def connDB():
	...
	user['eventPeople'+(str)(eventsNum)] = nowRecord[2]
	user['eventTime'+(str)(eventsNum)] = nowRecord[3]
	user['eventDate'+(str)(eventsNum)] = date
	user['allEventsNum'] = eventsNum
...
def createResponse(user):
	...
	if trackLevel == 1: #第一層
		...
		if trackLevel == 1:
			trackLevel+=1
			chatbotResponse = firstLevel(user,oriLeafTagName)
